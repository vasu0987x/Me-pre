...
seen_messages = set()
group_queue = queue.Queue()    # For group messages
personal_queue = queue.Queue()  # For personal messages
...
def sender_worker():
    while True:
        if not personal_queue.empty():
            item = personal_queue.get()
            msg, chat_id, receive_time = item
            ...
            personal_queue.task_done()
            time.sleep(0.1)  # Faster for personal chats
        elif not group_queue.empty():
            item = group_queue.get()
            msg, chat_id, kb, receive_time = item
            success = send_to_telegram(msg, chat_id, kb)
            if success:
                print(f"Sent group msg to {chat_id} at {time.time()}, delay: {time.time() - receive_time}s", flush=True)
            group_queue.task_done()
            time.sleep(1.1)  # Respect 1 msg/sec for group
        else:
            time.sleep(0.1)  # Idle
...
def format_message(record, personal=False):
    ...
    receive_time = time.time()  # Track when OTP is received
...
def main_loop():
    ...
    while True:
        stats = safe_request(f"{BASE_URL}/viewstats", {
            ...
            "records": 50  # Increased to handle bursts
        }) or {}
        if stats.get("status") == "success":
            for record in stats["data"]:
                ...
                if len(seen_messages) > 100000:  # Prevent memory bloat
                    seen_messages.clear()
                ...
                group_queue.put((msg_group, OTP_GROUP_ID, kb, receive_time))
                chat_id = user_numbers.get(number)
                if chat_id:
                    msg_personal, _, _, receive_time = format_message(record, personal=True)
                    personal_queue.put((msg_personal, chat_id, receive_time))
...
def queue_status(message):
    if message.from_user.id != ADMIN_ID:
        return bot.reply_to(message, "❌ You are not the admin.")
    bot.reply_to(message, f"📥 Group queue size: {group_queue.qsize()}\n📥 Personal queue size: {personal_queue.qsize()}")
