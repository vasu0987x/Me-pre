    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"❌ Telegram Error ({chat_id}): {r.text}", flush=True)
...
def main_loop():
    ...
    while True:
        ...
        if stats.get("status") == "success":
            # Buffer to hold messages for this fetch cycle
            messages_to_send = []
            for record in stats["data"]:
                ...
                if chat_id:
                    ...
                    print(f"⚠️ No user found for number {number}, skipping personal DM", flush=True)
            
            # Send all messages with 0.2s rate limit
        ...
        # Ensure next fetch is 0.2s after the last
...
def queue_status(message):
    if message.from_user.id != ADMIN_ID:
        return bot.reply_to(message, "❌ You are not the admin.")
    bot.reply_to(message, "📥 No queue used. Messages sent directly with 0.2s rate limit.")
