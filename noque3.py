...
def send_to_telegram(msg, chat_id=OTP_GROUP_ID, kb=None):
    ...
    for attempt in range(3):
        try:
            ...
            end_time = time.time()
            print(f"📤 Telegram API call to {chat_id} took {end_time - start_time:.2f}s", flush=True)
            if r.status_code == 200:
                ...
                print(f"❌ Telegram Error ({chat_id}): {r.status_code} - {r.text}", flush=True)
        except Exception as e:
            print(f"❌ Telegram Exception ({chat_id}): {e}, attempt {attempt + 1}/3", flush=True)
            time.sleep(1)
    return success
...
def handle_document(message):
    ...
    numbers = [line.strip().lstrip("0").lstrip("+") for line in downloaded_file.decode("utf-8").splitlines() if line.strip()]
...
def main_loop():
    ...
    while True:
        ...
        stats = safe_request(f"{BASE_URL}/viewstats", {
            ...
            "records": 5  # Reduced to minimize load
        }) or {}
        if stats.get("status") == "success":
            ...
            for record in stats["data"]:
                ...
                print(f"Received OTP for {number} at {fetch_time:.2f}s", flush=True)
                
                msg_group, kb, _ = format_message(record, personal=False)
                messages_to_send.append((msg_group, OTP_GROUP_ID, kb, fetch_time))
                
                chat_id = user_numbers.get(number)
                if chat_id:
                    msg_personal, _, _ = format_message(record, personal=True)
                    messages_to_send.append((msg_personal, chat_id, None, fetch_time))
                    print(f"🌀 Prepared personal DM for {number} to {chat_id} at {fetch_time:.2f}s", flush=True)
                else:
                    print(f"⚠️ No user found for number {number} in user_numbers, skipping personal DM", flush=True)
            
            for msg, chat_id, kb, fetch_time in messages_to_send:
                current_time = time.time()
                elapsed = current_time - last_send_time
                if elapsed < 0.3:  # Increased to 0.3s to reduce load
                    time.sleep(0.3 - elapsed)
                print(f"🌀 Sending to {chat_id} at {current_time:.2f}s", flush=True)
                success = send_to_telegram(msg, chat_id, kb)
                last_send_time = time.time()
                if success:
                    print(f"📤 Sent to {chat_id} at {last_send_time:.2f}s, delay from fetch: {last_send_time - fetch_time:.2f}s", flush=True)
                else:
                    print(f"❌ Failed to send to {chat_id} after retries", flush=True)
        
        time_to_next_fetch = fetch_time + 0.2 - time.time()
        if time_to_next_fetch > 0:
...
def queue_status(message):
    if message.from_user.id != ADMIN_ID:
        return bot.reply_to(message, "❌ You are not the admin.")
    bot.reply_to(message, "📥 No queue used. Messages sent directly with 0.3s rate limit.")
...
def send_random_number(chat_id, country=None, edit=False):
    ...
    number = number.lstrip("0").lstrip("+")  # Normalize number
    user_current_country[chat_id] = country
    user_numbers[number] = chat_id
    print(f"Assigned number {number} to chat_id {chat_id}", flush=True)
...
@bot.message_handler(commands=["debugnumbers"])
def debug_numbers(message):
    if message.from_user.id != ADMIN_ID:
        return bot.reply_to(message, "❌ You are not the admin.")
    text = "📋 user_numbers:\n" + "\n".join(f"{num}: {chat_id}" for num, chat_id in user_numbers.items())
    bot.reply_to(message, text or "❌ No numbers assigned.")
...
- /debugnumbers: Show assigned numbers and chat IDs
"""
