    ...
    while True:
        ...
        if stats.get("status") == "success":
            for record in stats["data"]:
                ...
                print(f"🌀 Queued group msg for {number}", flush=True)
                chat_id = user_numbers.get(number)
                if chat_id:
                    msg_personal, _, _ = format_message(record, personal=True)
                    message_queue.put((msg_personal, chat_id, None))
                    print(f"🌀 Queued personal msg to {chat_id}", flush=True)
        time.sleep(0.2)  # Match seven2.py fetch speed
...
def queue_status(message):
    if message.from_user.id != ADMIN_ID:
        return bot.reply_to(message, "❌ You are not the admin.")
    bot.reply_to(message, f"📥 Queue size: {message_queue.qsize()}")
