...
seen_messages = set()
send_queue = Queue()       # Holds (msg, chat_id, kb, retry_count)
last_global_send = 0
last_group_send = 0
last_user_sends = {}       # chat_id: last_send_time
    ...
    for attempt in range(5):  # Increased to 5 retries
        try:
            ...
            if r.status_code == 200:
                print(f"✅ Sent to {chat_id} (attempt {attempt+1})", flush=True)
                return True, None
            elif r.status_code == 429:
                wait_time = 2 * (2 ** attempt)  # 2s, 4s, 8s, 16s, 32s
                print(f"⚠️ 429 Rate limit for {chat_id}. Waiting {wait_time}s (attempt {attempt+1}/5)", flush=True)
                time.sleep(wait_time)
            else:
                print(f"❌ Failed for {chat_id}: status {r.status_code}, {r.text} (attempt {attempt+1}/5)", flush=True)
        except Exception as e:
            print(f"❌ Exception for {chat_id}: {e} (attempt {attempt+1}/5)", flush=True)
            time.sleep(0.2)
    print(f"❌ Failed to send to {chat_id} after 5 attempts: {msg[:100]}...", flush=True)
    return False, str(r.status_code) if 'r' in locals() else "Exception"

# ---------------- SENDER WORKER ----------------
def sender_worker():
    global last_global_send, last_group_send, last_user_sends
    while True:
        msg, chat_id, kb, retry_count = send_queue.get()
        now = time.time()

        # Calculate rate limit waits
        global_wait = max(0, 0.04 - (now - last_global_send))  # ~25/sec global
        if chat_id == OTP_GROUP_ID:
            group_wait = max(0, 1.0 - (now - last_group_send))  # 1/sec for group
            total_wait = max(global_wait, group_wait)
        else:
            last_user_time = last_user_sends.get(chat_id, 0)
            user_wait = max(0, 3.0 - (now - last_user_time))  # ~20/min per user
            total_wait = max(global_wait, user_wait)

        if total_wait > 0:
            time.sleep(total_wait)
            now = time.time()

        success, error = send_to_telegram(msg, chat_id, kb)
        if success:
            last_global_send = now
            if chat_id == OTP_GROUP_ID:
                last_group_send = now
            else:
                last_user_sends[chat_id] = now
            print(f"📤 Sent to {chat_id} (retries: {retry_count})", flush=True)
        else:
            send_queue.put((msg, chat_id, kb, retry_count + 1))
            print(f"🔄 Re-queued for {chat_id} (retry {retry_count + 1}, reason: {error})", flush=True)
...
def main_loop():
    ...
    while True:
        stats = safe_request(f"{BASE_URL}/viewstats", {
            ...
            "records": 10  # Increased for bursts
        }) or {}
        if stats.get("status") == "success":
            for record in stats["data"]:
                ...
                # Queue group message
                msg_group, kb, _, _ = format_message(record, personal=False)
                send_queue.put((msg_group, OTP_GROUP_ID, kb, 0))
                
                # Queue personal message
                chat_id = user_numbers.get(number)
                if chat_id:
                    msg_personal, _, _, _ = format_message(record, personal=True)
                    send_queue.put((msg_personal, chat_id, None, 0))
...
def queue_status(message):
    ...
    bot.reply_to(message, f"📥 Send queue size: {queue_size} (no skips, auto-retries enabled).")
...
- /queuestatus: Show queue status (no skips, auto-retries enabled)
"""
